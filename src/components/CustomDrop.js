...
const CustomDrop = ({ data, val, setVal, place, label, styleMain }) => {
    ...
    return (
        <View style={[{ marginHorizontal: 18 }, styleMain]}>
            {label && <Text style={[Commonstyles(Appcolor).mediumText12, { marginBottom: 4 }]}>{label}</Text>}
            <Dropdown
                ...
                renderItem={(data) => {
                    return (
                        <View style={{ paddingVertical: 8, paddingHorizontal: 16, backgroundColor: data?.value == value ? Appcolor.whiteop : Appcolor.white }}>
                            <Text style={{ color: Appcolor.txt, fontSize: 16, fontFamily: Appfonts.regular }}>{data?.label}</Text>
                        </View>
                    )
                }}
                renderRightIcon={() => (
