    ...
    return (
        ...
        >
            ...
            <View style={{ width: "57%", maxWidth: "57%" }}>
                <Text style={[styles.txtBig, { color: Appcolor.txt }]}>{item?.user?.full_name} {item?.user?.user_type == "brand" && `(${item?.user?.parent_brand_detail?.brand_name ?? ""})`}</Text>
                <Text style={[styles.txtSmall, { marginTop: 4, color: Appcolor.txt }]} numberOfLines={1}>{item?.lastMessage.text}</Text>
            </View>
            <View style={{ alignItems: 'center', width: "20%", height: "100%" }}>
                <Text style={[styles.txtSmall, { fontSize: 8, color: Appcolor.txt, fontFamily: Appfonts.semiBold, marginBottom: 4 }]}>{moment(item.lastMessage._id * 1000).fromNow()}</Text>
