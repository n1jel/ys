...
import LinkBlock, { LinkIcon } from '../../components/LinkBlock';
...
const ProductScreen = ({ navigation, route }) => {
  ...
  console.log(route?.params?._item, '----------t----');
  const preloadImages = () => {
  ...
  //     if (preloadedCount === allMedia.length) {
  //         setPreloadingComplete(true);

  //         if (allMedia.length < 15){
  //             setTimeout(() => {
  ...
  //         }
  //     }
  // }, [preloadedCount, allMedia.length]);
  ...
  const optSelected = async type => {
    if (type == 'library') {
      //  navigation.navigate("EditCollectionGallery", { collectionId: data?._id, })
      ...
      //   navigation?.navigate("ImageSelectionGallery", { collectionId: data?._id, })
    } else if (type == 'camera') {
      if (isUploadingNative) {
        Alert.alert('Upload in progress', 'Please wait for current uploading');
      } else {
        navigation?.navigate('ImageSelectionGallery', {
    ...
    <View style={{ flex: 1, backgroundColor: Appcolor.white }}>
      <ImageBackground
        ...
        <Header
          ...
          // showPin={(isMine && !deleteMode && !showVertical)}
          // onPressPin={() => setCoverSelectionMode(true)}
        ...
        <JacketModal
          ...
          isMine={isMine}
        />
        {/* {(isMine && !showVertical && coverSelectionMode) && <Text style={[Commonstyles(Appcolor).mediumText14, { marginHorizontal: 16, marginTop: 16, textAlign: 'center' }]}>Tap on any image to set collection Cover image</Text>} */}
...
export const N3Block = ({
  ...
  const [useNormalImage, setUseNormalImage] = useState(false)
      ...
      <View style={{ justifyContent: 'center', alignItems: 'center' }}>
        ...
        {media_type == 'video' && !thumbnail ? (
          ...
          <FastImage
            ...
            fallback={Platform?.OS == "android" ? useNormalImage : false}
...
export const VerticalBlock = ({ item, onPressVideo, brand, onPressLike, onMessagePress }) => {
  console.log('in vertical block');
  const { Appcolor } = useTheme();
  const { media_name, media_type, thumbnail, like_data } = item?.gallery_data;
  ...
  return (
    <View
      ...
      {media_type == 'video' && !thumbnail ? (
        ...
        <Pressable
          ...
          <FastImage
            resizeMode="cover"
            source={{
              uri: fileUrl + (media_type == 'image' ? media_name : thumbnail),
            }}
            style={{
              width: widthPercentageToDP(100),
              height: heightPercentageToDP(60),
      ...
      />
      {brand &&
        <View style={[Commonstyles(Appcolor).row, { marginTop: 8, paddingHorizontal: 12, width: "100%", justifyContent: "space-between" }]}>
          <Pressable onPress={() => onMessagePress()} style={[Commonstyles(Appcolor).row]}>
