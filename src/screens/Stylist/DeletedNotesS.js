...
import { DeleteClientNotes, DeleteNotes, clientRecentlyDeletedNotes, restore_note, restore_note_stylist, stylistRecentlyDeletedNotes } from '../../apimanager/httpServices'
...
const DeletedNotesS = ({ navigation }) => {
    ...
    const getNotes = async () => {
        try {
            ...
            let res = await stylistRecentlyDeletedNotes()
            if (res?.data?.status) {
    ...
    const restoreNote = async (note_id) => {
        try {
            let res = await restore_note_stylist({ note_id })
            if (res?.data?.status) {
                removeItem(note_id)
            } else {
                showToast(res?.data?.message ?? "Something went wrong.")
