        ...
        if (engine == null) {
            ...
            await getAuthToken(config.uid, config.channelId).then(async (res) => {
                ...
                if (token) {
                    ...
                    let res = await SendNotification({ sender_user_id: user?._id, user_type: otherUser?.user_type, user_id: otherUser._id, payload: { title: user.full_name, body: 'Audio Calling', sound: 'calling.mp3', channel_id: "call", android_channel_id: "call", data: { user_type: 'stylist', channelId: channelId, type: 'audio', profile: user?.profile_pic, name: user?.full_name, user_id: user._id } } })
        ...
        <ImageBackground style={{ flex: 1 }} source={Appimg.backgroundCall}>
            ...
            <View style={{ flexDirection: "row", alignSelf: "center", width: "100%", justifyContent: "space-evenly" }}>
                ...
                <Pressable onPress={async () => {
                    leaveChannel()
                    let res = await SendNotification({ sender_user_id: user?._id, user_type: otherUser?.user_type, user_id: otherUser._id, payload: { title: user?.full_name, body: 'Call Declined', data: { user_type: 'stylist', channelId: channelId, type: 'call-declined', profile: user?.profile_pic, name: user?.full_name, user_id: user._id } } })
