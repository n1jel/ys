...
const SettingS = ({ navigation }) => {
    ...
    const [list, setList] = useState([
        { title: 'Profile', onPress: () => navigation.navigate("ProfileS") },
        { title: 'Change Password', onPress: () => navigation.navigate("ChangePasswordS") },
        { title: 'Contact Us', onPress: () => navigation.navigate("ContactusS") },
        { title: 'Notification' },
        Platform.OS == "ios" ? { title: 'Biometrics' } : {},
        // { title: 'Dark Theme' },
        { title: 'Confirmed Orders', onPress: () => { navigation.navigate("ConfirmedOrders") } },
        // { title: 'Subscription', onPress: () => navigation.navigate("Subscripton", { from: "profile" }) },
        // { title: 'Add Stylist', onPress: () => navigation.navigate("StylistAccount") },
    ])
    const [list2, setList2] = useState([
        { title: "Terms & Services", onPress: () => navigation.navigate("TermsS", { from: "terms" }) },
        { title: 'Privacy Policy', onPress: () => navigation.navigate("TermsS", { from: "privacy" }) },
        { title: 'Delete Account', onPress: () => { navigation.navigate("DeleteAccountS") } },
    ])
    ...
    async function on_click() {
        try {
            let res = await UpdateNotStylist({ 'notification_status': user?.notification_status == '0' && user?.notification_status != undefined ? 1 : '0', 'fcm_token': user?.notification_status == '0' && user?.notification_status != undefined ? fcmtoken : '' });
            if (res?.data?.status) {
        ...
        <View style={{ flex: 1, backgroundColor: Appcolor.white }}>
            <ImageBackground source={theme == 'light' ? Appimg?.bg : Appimg.darkbg1} style={{ flex: 1 }}>
                ...
                <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
                    <Pressable onPress={() => { navigation.navigate('Notes') }} style={{ borderRadius: 10, marginTop: 20, height: 90, marginHorizontal: 16, paddingHorizontal: 15, backgroundColor: theme == 'dark' ? "#22222280" : "#FEFBF4", flexDirection: 'row', justifyContent: 'space-between', alignItems: "center", marginBottom: 4 }}>
                        <View>
