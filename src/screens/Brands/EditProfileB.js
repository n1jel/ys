...
import { updateProfileBrand } from '../../apimanager/brandServices'
import { getData } from '../../utils/asyncstore'


const EditProfileB = ({ navigation }) => {
    ...
    const updateProfile = async () => {
        try {
            ...
            let obj = [
                ...
                { name: "phone_number", data: phone },
                { name: "country_code", data: countryCode },
            ];
            console.log(newPic);
            if (newPic != null) {
                let finalPath = RNFetchBlob.wrap(newPic?.path);
                let temp = {
                    filename: newPic.filename == null ? Date.now() + "uff.jpg" : newPic.filename,
                    type: newPic.mime,
                    data: finalPath,
                    name: "profile_pic",
                };
            ...
            let resp = await updateProfileBrand(obj, token)
            ...
            if (res?.status) {
                ...
                showToast(res?.message ?? "Network error")
            }
        } catch (e) {
            console.error(e);
        } finally {
        ...
        <ImageBackground style={{ flex: 1, backgroundColor: Appcolor.white }} source={theme == 'light' ? Appimg?.bg : Appimg.darkbg1}>
            ...
            >
                ...
                <Btn title={en?.changepassword} transparent={Appcolor.primary} twhite styleMain={{ alignSelf: "center", marginTop: 20 }}
                    onPress={() => navigation.navigate("ChangePasswordB")}
                />
            </ScrollView>
        </ImageBackground>
    )
