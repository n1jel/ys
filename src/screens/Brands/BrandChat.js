...
import { confirmOrders } from '../../apimanager/brandServices'

const BrandChat = ({ navigation, route }) => {
    ...
    const onSend = useCallback(async (messages = []) => {
        try {
            ...
            if (userMine?.is_added_by_brand == 1) {
                u = { ...u, parent_brand_detail: userMine?.parent_brand_id }
            }
            let message = {
                _id: moment().unix(),
            ...
            let res = await SendNotification({ sender_user_id: userMine?._id, user_type: userMine?.is_added_by_brand == 1 ? "client" : 'stylist', user_id: user._id, payload: { title: 'New Message', body: userMine?.full_name + ' sent you a message', channel_id: "call", android_channel_id: "call", data: { user_type: 'stylist', type: 'chat', profile: userMine?.profile_pic, name: userMine?.full_name, user_id: userMine._id } } })
            console.log(res);
        } catch (e) {
            console.error(e);
        }
    ...
    async function on_confirm() {
        ...
        try {
            ...
            let res = await confirmOrders(_data)
            if (res?.data?.status) {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: Appcolor.white }}>
            <LinearGradient colors={[Appcolor.grad1, Appcolor.primary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[styles.headerView]}>
                <View style={{ flexDirection: "row", width: widthPercentageToDP(60) }}>
                    <View style={{ flexDirection: "row", alignItems: "center", marginLeft: 16 }}>
                        ...
                        <Pressable onPress={() => {
                            if (userMine?.is_added_by_brand == 1) {
                                navigation?.navigate("ProfileClientStylist", { uId: user?._id, userType: 'client' })
                            } else {
                                navigation?.navigate("ProfileClientStylist", { uId: user?._id, userType: 'stylist' })
                            }
                        }}>
                ...
                <View style={{ flexDirection: "row", width: widthPercentageToDP(40), justifyContent: "center" }}>
                    <Pressable onPress={() => { navigation.navigate("CallBrands", { otherUser: user }) }}>
                        <Image source={Appimg.call} style={styles.Call} />
                    </Pressable>

                    <Pressable style={{ marginLeft: 16 }} onPress={() => { navigation.navigate("VideoCallBrands", { otherUser: user }) }}>
            ...
            <ConfirmOrder vis={showConfirmOrder}
                ...
                isParentBrand={userMine?.is_added_by_brand == 0 ? true : false}
            />
            <SectionList
