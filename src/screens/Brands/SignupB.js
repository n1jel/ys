...
import { emailExists, phoneExists } from '../../apimanager/brandServices'

const SignupB = ({ navigation }) => {
    ...
    const submit = async () => {
        ...
        try {
            ...
            let res = null;
            res = await emailExists(email);
            if (res?.data?.status) {
                let res = null;
                res = await phoneExists(phone_number);
                if (res?.data?.status) {
                    navigation.navigate("AddProfileB", { fullname: fullname, email: email?.trim(), phone_number: phone_number, password: password, countryCode: countryCode, confirmpassword: confirmpassword, country: country?.name, state: state?.name, city: cities?.name })
                } else {
            ...
            <ImageBackground source={theme == 'light' ? Appimg?.bg : Appimg.darkbg} style={{ flex: 1 }}>
                ...
                <KeyboardAvoidingView behavior={Platform?.OS == "ios" ? "padding" : null} keyboardVerticalOffset={60} >
                    <ScrollView showsVerticalScrollIndicator={false}>
                        <Pressable onPress={() => { resetData(); navigation?.goBack() }} style={{ marginHorizontal: 16, marginVertical: 20 }}>
                        ...
                        <Tinput value={fullname} onChangeText={(t) => setFullname(t)} place={en?.bname} styleMain={{ marginTop: 16 }} img={Appimg.user} />
                        ...
                        <Btn
                            ...
                            title={en?.next}
                            twhite
                            styleMain={{ marginTop: 60, alignSelf: "center" }}
                            onPress={() => {
                                submit()
                            }}
                        />
                        <Text style={[Commonstyles(Appcolor)?.regular12, { marginHorizontal: 18, marginTop: 8, textAlign: "center", marginBottom: 30 }]} onPress={() => { resetData(); navigation.navigate("LoginB"); }}>{en?.alreadyanaccount}?<Text style={{ color: Appcolor.primary, fontFamily: Appfonts.bold, fontSize: 14 }}> {en?.login}</Text></Text>
                    </ScrollView>
