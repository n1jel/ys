...
import { resetPassBrand } from '../../apimanager/brandServices'

const NewPasswordB = ({ navigation, route }) => {
    ...
    async function ResetPass() {
        try {
            ...
            let res = await resetPassBrand({ _id: route?.params?.id, password });
            if (res?.data?.status) {
                setTimeout(() => {
                    setSucessModal(true);
                }, 800);
        ...
        <SafeAreaView edges={["top"]} style={{ flex: 1, backgroundColor: Appcolor.white }}>
            ...
            <ImageBackground source={Appimg?.darkbg} style={{ flex: 1 }}>
                <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    <Btn transparent={Appcolor.blackop} title={en?.confirm} twhite styleMain={{ marginTop: 200, alignSelf: "center" }}
                        onPress={() => {
                            if (password == "") {
