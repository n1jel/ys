...
import { getClientBrandCollection, getClientBrandDetail, getClientStylistCollection, getClientStylistDetail, getConfirmedOrders, getConfirmedOrdersBrand, SendFollowRequest, SendFollowRequestBrand } from '../../apimanager/httpServices'
...
const BrandProfile = ({ navigation, route }) => {
    ...
    async function get_detail() {
        try {
            ...
            let res = await getClientBrandDetail(id)
            if (res?.data?.status) {
    ...
    async function get_collection(item) {
        ...
        try {
            ...
            let res = await getClientBrandCollection(item?._id, type, 1, limit)
            if (res?.data?.status) {
    ...
    async function get_more_collection(item, page) {
        ...
        try {
            setAlbumsLoading(true)
            let res = await getClientBrandCollection(item?._id, type, page, limit)
            if (res?.data?.status) {
                let temp = [...albums]
    ...
    const getOrders = async (item) => {
        try {
            let res = await getConfirmedOrdersBrand({ brand_id: item, page: 1, limit })
            if (res?.data?.status) {
    ...
    const getMoreOrders = async (item, page) => {
        try {
            let res = await getConfirmedOrdersBrand({ brand_id: item?._id, page, limit })
            if (res?.data?.status) {
                let temp = [...orders]
                temp = temp?.concat(res?.data?.data)
                setOrders(temp)
    ...
    async function send_request() {
        try {
            dispatch(setLoading(true))
            let _data = { 'brand_id': detail?._id }
            let res = await SendFollowRequestBrand(_data)
            console.log(res, 'SendFollowRequest')
            if (res?.data?.status) {
                get_detail()
        ...
        <ImageBackground source={theme == 'light' ? Appimg?.bg : Appimg.darkbg1} style={{ flex: 1, backgroundColor: Appcolor.white }}>
            ...
            {(detail && detail?._id && mode == 0) &&
                <>
                    ...
                    <StylistBlock item={detail} styleMain={{ marginRight: 0 }} isBrand onPress={() => { }} />
