...
const VideoCallClient = ({ navigation, route }) => {
    ...
    const _initEngine = async () => {
        ...
        await getAuthToken(config.uid, config.channelId).then(async (res) => {
            ...
            if (token) {
                ...
                let res = await SendNotification({ sender_user_id: user?._id, user_type: otherUser?.user_type, user_id: otherUser._id, payload: { title: user.full_name, body: 'Video Calling', sound: 'calling.mp3', channel_id: "call", android_channel_id: "call", data: { user_type: 'client', channelId: channelId, type: 'video', profile: user?.profile_pic, name: user?.full_name, user_id: user._id } } })
    ...
    useEffect(() => {
        console.log('DattaaaaaaacallState', callState)
        if (!callState) {
            console.log('DattaaaaaaacurrentNotification', currentNotification)
    ...
    return (
        <>
            ...
            <Pressable style={{ position: 'absolute', bottom: 30, alignSelf: 'center' }} onPress={async () => {
                leaveChannel()
                let res = await SendNotification({ sender_user_id: user?._id, user_type: otherUser?.user_type, user_id: otherUser._id, payload: { title: user?.full_name, body: 'Video call Declined', data: { user_type: 'client', channelId: channelId, type: 'call-declined', profile: user?.profile_pic, name: user?.full_name, user_id: user._id } } })
