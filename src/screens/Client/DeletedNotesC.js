...
import { DeleteClientNotes, clientRecentlyDeletedNotes, restore_note } from '../../apimanager/httpServices'
...
const DeletedNotesC = ({ navigation }) => {
    ...
    const getNotes = async () => {
        try {
            ...
            let res = await clientRecentlyDeletedNotes()
            if (res?.data?.status) {
    ...
    const permanentlyDelete = async (note_id) => {
        try {
            dispatch(setLoading(true))
            let res = await DeleteClientNotes(note_id, 1)
            if (res?.data?.status) {
                removeItem(note_id)
