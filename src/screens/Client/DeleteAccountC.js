...
import { deleteAccountClient } from '../../apimanager/httpServices'
...
const DeleteAccountC = ({ navigation }) => {
    ...
    const submit = async () => {
        ...
        try {
            ...
            let res = await deleteAccountClient(body)
            if (res?.data?.status) {
                dispatch(signout())
    ...
    return (
        <ImageBackground style={{ flex: 1, backgroundColor: Appcolor.white }} source={theme == 'light' ? Appimg?.bg : Appimg.darkbg1}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                />
                <Btn transparent={Appcolor.primary} twhite title={en?.deleteaccount} styleMain={{ marginTop: 100, alignSelf: "center" }}
                    onPress={() => {
