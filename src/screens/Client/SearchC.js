...
import { getClientStylist, getStylistandBrands, SendFollowRequest, SendFollowRequestBrand } from '../../apimanager/httpServices'
...
const SearchC = () => {
    ...
    async function get_stylist(page, limit, text) {
        ...
        let body = { page, limit, text }
        try {
            dispatch(setLoading(true))
            let res = await getStylistandBrands(`?page=${page}&limit=${limit}&keyword=${text}&type=both`)
            if (res?.data?.status) {
    ...
    async function sendReqBrand(item) {
        try {
            ...
            console.log(res, 'SendFollowRequestToBrand')
            if (res?.data?.status) {
                get_stylist(1, 100, search)
            } else {
