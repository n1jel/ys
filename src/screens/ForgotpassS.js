...
const ForgotpassS = ({ navigation }) => {
  ...
  async function handle_forgotpass() {
    try {
      ...
      if (email) {
        let res = null;
        res = await forgotPasswordStylist(email, countryCode);
        if (res?.data?.status) {
          showToast(res?.data?.message ?? "otp sent succesfully");
          navigation.navigate("VerificationS", { email });
        } else {
      ...
      <ImageBackground source={Appimg?.darkbg} style={{ flex: 1 }}>
        <KeyboardAwareScrollView showsVerticalScrollIndicator={false}>
          ...
          {/* <Tinput onChangeText={(t) => setEmail(t)}
            place={en?.email} styleMain={{ marginTop: 26 }} img={Appimg?.mail} /> */}
          {/* <Tinput onChangeText={(t) => setEmail(t)}
