...
class S3UploadManager {
  ...
  private let uploadQueue = DispatchQueue(label: "org.solidappmaker.YourSeason.s3UploadQueue")
  private var imagesDataToUpload = [Data]()
  private var bucketName = "yourseason"
  private var mime_type = "image/jpeg"
  private var uploadedImageKeys = [String]() // Array to store uploaded image keys
  var collectionID = ""
  var uploadedImageKeysString = ""
  ...
  @objc func cancel(_ notification: Notification) {
    print("cancelling")
    self.pickerParameters = nil
    self.cancelProcess = true
    self.cancelAllS3Uploads()
    self.cancelAllAPIRequests()
    self.imagesDataToUpload.removeAll()
    self.uploadedImageKeys.removeAll()
    self.assets.removeAll()
    self.collectionID = ""
    self.uploadedImageKeysString = ""
    let count = self.imagesDataToUpload.count
  ...
  func addImageDataToUpload(_ imageData: Data) {
    imagesDataToUpload.append(imageData)
  }
     ...
     print(collectiondata, "????????????????????")
      ...
      print("Update Collection Case in already created collection")
        ...
        if collectiontype == "Brands"{
          updateMedia(type: collectiontype!) {
            completion()
          }
        }else{
          updateMedia(type: "") {
...
//    print(pickerParameters?.collectionData,"here is collectiondata ",pickerParameters?.collectionData.isEmpty)
//    return
 
  }
  
  
  func toCreateCollection(collectiontype:String ,completion: @escaping () -> Void){
    let dict: Dictionary<String,Any> = [
      "name": "Uploading",
      "total": String(self.assets.count),
      "completed":String(self.uploadedImageKeys.count),
      "remaining" : String((self.assets.count) - (self.uploadedImageKeys.count)),
    ]
    NotificationCenter.default.post(name: NSNotification.Name("status"), object: self, userInfo: ["data": dict])
    ...
    createCollection(type: collectiontype) { [weak self] in
      ...
      uploadBatch = { [weak self] in
        ...
        if currentBatchIndex < totalBatches {
          ...
          self.updateCollection(type: collectiontype) {
  ...
  func addMediaToGallery(completion: @escaping () -> Void){
      let dict: Dictionary<String,Any> = [
        "name": "Uploading",
        "total": String(self.assets.count),
        "completed":String(self.uploadedImageKeys.count),
  ...
  func updateMedia(type:String,completion: @escaping () -> Void){
    let dict: Dictionary<String,Any> = [
      "name": "Uploading",
      "total": String(self.assets.count),
      "completed":String(self.uploadedImageKeys.count),
    ...
    backgroundTask = UIApplication.shared.beginBackgroundTask {
      ...
      uploadBatch = { [weak self] in
        ...
        if currentBatchIndex < totalBatches {
          ...
          if type == "Brands"{
            self.updateCollection(type: "Brands") {
              DispatchQueue.main.async {
                UIApplication.shared.endBackgroundTask(self.backgroundTask)
                self.backgroundTask = .invalid
              }
    ...
    for (offset, imageData) in batch.enumerated() {
      ...
      transferUtility?.uploadData(sentToData, bucket: bucketName, key: key, contentType: contentType, expression: nil, completionHandler: { [weak self] (task, error) in
        ...
        if let error = error {
          ...
          let dict: Dictionary<String,Any> = [
            "name": "Uploading",
            "error":"Not able to upload Error by Aws \(error.localizedDescription)"
          ]
          NotificationCenter.default.post(name: NSNotification.Name("status"), object: self, userInfo: ["data": dict])
          dispatchGroup.leave()
        } else {
  ...
  private func createCollection(type:String,completion: @escaping () -> Void) {
    ...
    print("create collection type", type)
    ...
    for asset in assets {
      ...
      PHImageManager.default().requestImageDataAndOrientation(for: asset, options: options) { (imageData, _, _, info) in
        if let error = info?[PHImageErrorKey] as? Error {
          let dict: Dictionary<String,Any> = [
            "name": "Uploading",
            "error":"Could not get the image \(error.localizedDescription)"
          ]
          NotificationCenter.default.post(name: NSNotification.Name("status"), object: self, userInfo: ["data": dict])
          dispatchGroup.leave()
          return // Exit early since we've hit an error
        }
...
//
  ...
  private func updateCollection(type:String,completion: @escaping () -> Void) {
    print("i am started", type)
    if let dataParams  = pickerParameters{
      do {
        let jsonData = try JSONSerialization.data(withJSONObject: self.uploadedImageKeys, options: [])
        if let jsonString = String(data: jsonData, encoding: .utf8) {
          print("JSON string:======>>>> \(jsonString)")
          self.uploadedImageKeysString = jsonString
...
//          let urlString = "http://192.168.0.111:5068/api/v1/stylist/update_media_array"
  ...
  private func updateGallery(completion: @escaping () -> Void) {
    print("i am started update galary")
    if let dataParams  = pickerParameters{
      do {
        let jsonData = try JSONSerialization.data(withJSONObject: self.uploadedImageKeys, options: [])
        if let jsonString = String(data: jsonData, encoding: .utf8) {
          ...
          var temp = dataParams.additionalOptions
          temp["data"] =  jsonString
          let collectiontype = temp["collectionType"] as? String
          

          print(temp,"here is the data ",dataParams.token)
          print(temp,"here is the type ",collectiontype)
...
// Extension to get HTTP status code from URLResponse
