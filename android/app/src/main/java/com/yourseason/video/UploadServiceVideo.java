...
import com.yourseason.ImageUtils;
...
public class UploadServiceVideo extends Service {
    private List<SelectedVideo> selectedPhotos = new ArrayList<>();
    private List<SelectedVideo> selectedPhotosss = new ArrayList<>();
    ...
    private Queue<SelectedVideo> uploadQueueee = new LinkedList<>();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        selectedPhotos = (ArrayList<SelectedVideo>) intent.getSerializableExtra("queue");
        selectedPhotosss = (ArrayList<SelectedVideo>) intent.getSerializableExtra("queue");
        ...
        isCollectionIdFromReactNative = intent.getBooleanExtra("isCollectionIdFromReactNative", false);
    ...
    public static void stopService(Context context) {
        ...
        Intent serviceIntent = new Intent(context, com.yourseason.UploadService.class);
...
//    }
    private void uploadNextImage() {
        if (shouldContinue) {
            if (!uploadQueue.isEmpty() && !isUploading) {
                SelectedVideo photo = uploadQueue.poll();
                if (photo != null) {
                    isUploading = true;
                    String fileName = "gallery/test/" + System.currentTimeMillis() + photo.getName();
                    Log.w("CHECK uploadNextImage ",photo.getUri());
                    Log.w("CHECK DATA==mime >",photo.getMimetype());
                    Log.w("CHECK DATA==path  >",photo.getPath());
                    Log.w("CHECK DATA==name  >",photo.getName());
                    Log.w("CHECK DATA==uri  >",photo.getUri());
                    Log.w("CHECK DATA==filename >",fileName);
                    uploadImage(photo.getPath(), fileName);

//                    String compressedImagePath = VideoCompressor.compressVideo(this,photo.getUri());
...
//                    }
    ...
    private void getStausOfUploadedAndPendingimages(int pendingCount, int uploadedCount) {
        ...
        Log.w("shouldConti VIDEOss--", shouldContinue + " =======");
        if (shouldContinue) {
            Log.w("Uploading status VIDEOss", "\n");
            ...
            if (uploadedCount + failedPhotos == totalImageforUpload) {
                ...
                Log.d("VIDEOss UPLOADED  is ", getPathfromserver.toString());
            ...
            Log.w("VIDEOss MAP DATA", map.toString());
    ...
    public void hitapiGallery(List<String> list) {
        ...
        try {
            ...
            requestBody.put("media_type","video");
        } catch (JSONException e) {
            e.printStackTrace();
        }
        String url = "";
        if (isCollectionIdFromReactNative) {
            url = baseUrl+"update_media_array";
        } else {
            if (collectionIdValue.isEmpty()) {
                url = baseUrl+"update_gallery_media";
            } else {
                url = baseUrl+"update_media_array";
            }
        }
...
//        https://api.yourseasonapp.com/api/v1/stylist/update_media_array
        Log.w("CHECK URL==", url + " " + collectionIdValue);
